...
En este caso el packing es en GPU
*/
...
// el bit packing pero version kernel
__global__ void kernel_bit_packing(const uint8_t* X, uint32_t* X_packed, int n, int m) {
    int packed_cols = (m + 15) / 16;
    int i = blockIdx.y * blockDim.y + threadIdx.y; // individuo
    int j = blockIdx.x * blockDim.x + threadIdx.x; // grupo de 16 SNPs

    if (i >= n || j >= packed_cols) {
        return;
    }
    ...
    for (int k = 0; k < 16; ++k) {
        int snp_index = base + k;
        if (snp_index < m) {
            uint32_t snp = X[i * m + snp_index] & 0x03u;
            valor_packed |= snp << (2 * k);
        }
    }

    X_packed[i * packed_cols + j] = valor_packed;
}
...
int main(int argc, char *argv[]) {
    ...
    uint8_t *d_X_raw; // bit packing gpu
    uint32_t *d_X;
    float *d_normas;
    float *d_C;
    float *d_D2;
    ...
    CUDA_CHK(cudaMalloc(&d_X_raw, n*m*sizeof(uint8_t)));
    ...
    /* Copio X NO PACKED de host a device y mido tiempos */
    
    cudaEvent_t startMemcpy, stopMemcpy;
    cudaEventCreate(&startMemcpy);
    cudaEventCreate(&stopMemcpy);
    cudaEventRecord(startMemcpy);
    CUDA_CHK(cudaMemcpy(d_X_raw, h_X, n*m*sizeof(uint8_t), cudaMemcpyHostToDevice));
    ...
    /* Bit packing en GPU y su medicion de tiempos */
    
    dim3 blockPack(32, 8);
    dim3 gridPack((packed_cols + blockPack.x - 1) / blockPack.x, (n + blockPack.y - 1) / blockPack.y);
    cudaEvent_t startPack, stopPack;
    cudaEventCreate(&startPack);
    cudaEventCreate(&stopPack);
    cudaEventRecord(startPack);
    kernel_bit_packing<<<gridPack, blockPack>>>(d_X_raw, d_X, n, m);
    CUDA_CHK(cudaGetLastError());
    cudaEventRecord(stopPack);
    cudaEventSynchronize(stopPack);
    float tiempoPacking;
    cudaEventElapsedTime(&tiempoPacking, startPack, stopPack);
    printf("Bit packing GPU: %.3f ms\n", tiempoPacking);
